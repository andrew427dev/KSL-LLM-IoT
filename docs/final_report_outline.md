...
- IoT 특성: 카메라/GPIO 버튼/부저(입력) → 엣지 추론 → LCD/스피커(출력), 클라우드 학습-엣지 배포 사이클
- 차별점: ① 단어 나열이 아닌 LLM 자연어 문장 ② 사용자 선택형 문체(persona)
  ③ 버튼+비프 피드백의 접근성 설계 ④ 공개 데이터셋(AI Hub)과 런타임 카메라 간
...
| 운용 모델 (30 classes, 시연자 3명, AI Hub) | [TBD — 학습 완료 후] |
| TFLite inference latency | 0.41 ms (server CPU) / [TBD] ms (RPi) |
| End-to-end FPS on RPi 4B | [TBD] (target ≥20) |
| Model size | 740 KB |
...
4. 접근성 중심 인터페이스: 인식율 의존 트리거 제거(물리 버튼), 청각 피드백
