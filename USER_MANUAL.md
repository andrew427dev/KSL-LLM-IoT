...
- **신호음·LED**: 단어 인식 시 부저+LED **짧게 1회**, 문장 완료/재생 시 **길게 1회**(페르소나 전환은 1/2/3회). LED는 부저와 동기 점멸 — 농인 사용자를 위한 시각 신호
...
| 상태 LED | LED + 저항(220~330Ω) | GPIO22(BCM) 기본 — `LED_PIN`. 부저와 동기 점멸(농인 시각 피드백). 핀(물리 15)+저항 → LED(+), LED(−) → GND |
...
5. **완료 버튼**(GPIO5)을 누르면 Gemini가 자연어 문장을 만들어 LCD/스피커로 출력한다(부저+LED **길게 1회** — 단어 인식음과 구분). "완료" 수어도 동일하게 동작한다. **버퍼가 빈 상태에서 완료 버튼을 다시 누르면 마지막 문장을 다시 재생한다**(다시 듣기). **3초 이상 무동작 시에도 자동으로 문장이 생성된다**(기본 `SILENCE_TRIGGER_SEC=3.0`, 완료 버튼과 병행 — 버튼 전용을 원하면 `.env`에서 `0`으로 설정).
