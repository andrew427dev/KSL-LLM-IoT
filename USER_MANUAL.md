...
- **문장화 트리거**: **완료 버튼**(GPIO5) 또는 단어 "완료" 수행 (침묵 자동완료는 기본 비활성 `SILENCE_TRIGGER_SEC=0` — 초기화 버튼과 충돌 방지)
- **문체 선택**: 버튼 2개 — 정중·친근 (비프 1/2회로 확인). 친근은 반말체
- **초기화(undo) 버튼**(GPIO6): 단어 입력 중에는 마지막 단어 1개 제거(비프 짧게 3회), 문장 출력 후에는 현재 문체 문장만 다시 생성(비프 길게 1회)
- **신호음·LED**: 단어 인식 시 부저+LED **짧게 1회**, 문장 완료/재생 시 **길게 1회**(페르소나 전환은 1/2회). LED는 부저와 동기 점멸 — 농인 사용자를 위한 시각 신호
- **라이브 프리뷰**(선택): `KSL_STREAM=1`로 실행하면 같은 네트워크의 브라우저에서 `http://<RPi_IP>:8080/`로 인식 화면(카메라+랜드마크+버퍼)을 확인할 수 있다(헤드리스에서도 동작)
...
| 상태 LED | LED + 저항(220~330Ω) | GPIO22(BCM) 기본 — `LED_PIN`. 부저와 동기 점멸(농인 시각 피드백). 핀(물리 15)+저항 → LED(+), LED(−) → GND(물리 25, SPI0 SCLK 아래) |
| 푸시 버튼 ×4 | 모멘터리 택트 스위치 | 문장 완료 GPIO5(물리 29) / 초기화·undo GPIO6(31) / 정중 GPIO13(33) / 친근 GPIO19(35). 각 버튼은 핀과 GND(물리 6) 사이 연결 — 내부 풀업, 외부 저항 불필요 |
...
| `SILENCE_TRIGGER_SEC` | 무동작 자동 문장화(초). `0` = 비활성(완료 버튼 전용) — 초기화(undo) 버튼과 충돌 방지를 위해 기본 비활성 | `0` |
| `DUPLICATE_FILTER_SEC` | 같은 단어 재인식 차단 창(초) | `3.0` |
| `LCD_I2C_ADDRESS` | LCD 주소 | `0x27` |
| `BUZZER_PIN` | 부저 GPIO(BCM) | `17` |
| `LED_PIN` | 상태 LED GPIO(BCM) — 부저 동기 점멸 | `22` |
| `BUTTON_COMPLETE_PIN` | 문장 완료 버튼 GPIO(BCM) | `5` |
| `BUTTON_UNDO_PIN` | 초기화(undo) 버튼 GPIO(BCM) — 마지막 단어 제거 / 문장 후 재생성 | `6` |
| `BUTTON_PERSONA_POLITE_PIN` | 정중 문체 버튼 GPIO(BCM) | `13` |
| `BUTTON_PERSONA_FRIENDLY_PIN` | 친근 문체 버튼 GPIO(BCM) | `19` |
...
| `SENTENCE_PERSONA` | 문장 생성 문체 — `정중`/`친근` | `정중` |
...
3. 인식 시 **부저음 + LED 점멸(짧게 1회)** + LCD에 단어와 신뢰도가 표시됩니다. LED는 부저와 동기되어 소리를 못 들어도 인식 여부를 눈으로 확인할 수 있습니다.
4. 단어를 이어서 수행하면 버퍼에 누적됩니다. 같은 단어는 3초(`DUPLICATE_FILTER_SEC`) 이내 재인식이 무시된다 — 동일 단어를 연속 입력하려면 3초 이상 간격을 둔다.
5. **완료 버튼**(GPIO5)을 누르면 Gemini가 자연어 문장을 만들어 LCD/스피커로 출력한다(부저+LED **길게 1회** — 단어 인식음과 구분). "완료" 수어도 동일하게 동작한다. 한 번의 호출로 정중·친근 두 문체를 동시에 생성해 캐시하므로, **문체 전환 후 완료 버튼을 다시 누르면 추가 호출 없이 캐시된 문장을 즉시 재생한다**(다시 듣기·문체 전환 모두 0회 호출). 침묵 자동완료는 기본 비활성이다(`SILENCE_TRIGGER_SEC=0`, 필요 시 `.env`에서 초 단위로 활성화).
6. **초기화(undo) 버튼**(GPIO6): 단어를 입력하는 중에 누르면 **마지막 단어 1개를 버퍼에서 제거**한다(오인식 복구, 부저 짧게 3회, LCD 버퍼 즉시 갱신). 문장이 출력된 뒤(버퍼가 빈 상태)에 누르면 **의미가 다르게 나온 현재 문체 문장만 다시 생성**한다(부저 길게 1회 — 해당 문체 캐시만 갱신).
7. **출력 분리**: 음성(TTS)은 한국어 문장, LCD·화면 표시는 영어다 — 문자형 LCD(HD44780)는 한글을 렌더링하지 못한다. 인식 단어도 LCD에는 영어 라벨(예: 배고프다→hungry)로 표시된다. 한 줄(20칸)을 넘는 긴 영어 문장은 LCD에서 좌로 흐르는 전광판(marquee) 방식으로 표시된다.
8. **문체 버튼** 2개로 출력 문체를 즉시 전환한다 — 정중(비프 1회) / 친근(비프 2회, 반말체). 비프 횟수로 화면을 보지 않아도 적용 문체를 확인할 수 있고, LCD 첫 줄에 현재 문체(`Style: Polite`/`Friendly`)가 표시된다. 시작 문체는 `.env` `SENTENCE_PERSONA`. 모니터 연결(GUI) 모드에서는 `SPACE` 키 = 완료, `p` 키 = 문체 순환, `z` 키 = 초기화(undo).
...
| 날짜 | 내용 |
|------|------|
| 2026-06-21 | **버튼 재배치 + 초기화(undo) 버튼 추가**: 완료5 / **초기화6** / 정중13 / 친근19 (간단 문체 삭제 — 문체 2개로, §1.1·§2.1·§3.2). 초기화 버튼은 단어 입력 중=마지막 단어 제거, 문장 후=현재 문체 재생성(§3.2). **친근 문체를 반말체로 변경**. **침묵 자동완료 기본 비활성**(`SILENCE_TRIGGER_SEC=0`, undo 충돌 방지). **문체 전환·완료 재누름은 캐시 재생(추가 호출 0회)** — 완료 시 두 문체를 1회 호출로 동시 생성. LCD: 첫 줄에 현재 문체 표시, 긴 문장 전광판 스크롤. **라이브 프리뷰** `KSL_STREAM=1`(브라우저 `http://<RPi_IP>:8080/`, §0) |
| 2026-06-21 | **상태 LED 추가**(GPIO22, `LED_PIN`) — 부저와 동기 점멸하는 시각 피드백(농인 접근성, §1.1 결선). **문장 완료/재생 시 부저+LED 길게 1회**(단어 인식음과 구분). **완료 버튼 재누름(버퍼 빈 상태) = 마지막 문장 다시 재생**(§3.2) |
