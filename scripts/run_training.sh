...
# 예:
#   bash scripts/run_training.sh ~/aihub_dataset            # 전체 각도, stride 15
#   bash scripts/run_training.sh ~/aihub_dataset 10 F D     # 정면+위, stride 10
# 어휘 오버라이드(파이프라인 검증 등): KSL_LABELS_OVERRIDE="단어1,단어2" 환경변수.
# 산출물: ~/ksl-artifacts/ (ksl_model.tflite, confusion_matrix.png, 로그)
set -euo pipefail
...
PY="$VENV/bin/python"
ART="${ART:-$HOME/ksl-artifacts}"
mkdir -p "$ART"

echo "=== [1/5] 매칭 스캔 ==="
"$PY" convert_aihub.py --dataset "$DATASET" --scan 2>&1 | tee "$ART/scan.log"

echo "=== [2/5] 키포인트 → CSV 변환 (stride=$STRIDE) ==="
rm -rf data/landmarks data/augmented
if [ ${#ANGLES[@]} -gt 0 ]; then
    "$PY" convert_aihub.py --dataset "$DATASET" --stride "$STRIDE" --angles "${ANGLES[@]}"
else
    "$PY" convert_aihub.py --dataset "$DATASET" --stride "$STRIDE"
fi
