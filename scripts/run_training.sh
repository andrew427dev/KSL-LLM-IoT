...
# find -print -quit 사용 — `find | grep -q`는 pipefail에서 grep이 파이프를 먼저
# 닫으면 find가 SIGPIPE(141)로 실패해 매치가 있어도 오탐 중단한다 (2026-06-11 실측).
if [ -z "$(find "$DATASET" -maxdepth 6 -type d -name "NIA_SL_WORD*" -print -quit 2>/dev/null)" ]; then
